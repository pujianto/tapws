...
class Server:
    ...
    async def _on_shutdown(self, app: Application) -> None:
        for connection in list(self.ws_handler.connections):
    ...
    async def start(self):
        ...
        self.site = web.TCPSite(
            self.runner, self.config.host, self.config.port, ssl_context=self.config.ssl
        )
        self.loop.add_reader(self.device.fileno(), partial(self.ws_handler.broadcast))
